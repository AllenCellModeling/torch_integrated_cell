...
function Model:createAutoencoder()
    ...
    self.encoder:add(nn.VolumetricConvolution(512, 1024, kT, kW, kH, dT, dW, dH, padT, padW, padH))
    self.encoder:add(nn.View(1024*2*4*3))
    self.encoder:add(nn.BatchNormalization(1024*2*4*3))
    self.encoder:add(nn.PReLU())
    ...
    if self.nClasses > 0 then
        ...
        pred_label:add(nn.Linear(1024*2*4*3, self.nClasses))
    ...
    -- output for the structure features
        ...
        pred_other:add(nn.Linear(1024*2*4*3, self.nOther))
    ...
    if self.nLatentDims > 0 then
        ...
        noise:add(nn.Linear(1024*2*4*3, self.nLatentDims))
    ...
    self.decoder:add(nn.Linear(self.nClasses + self.nOther + self.nLatentDims, 1024*2*4*3))

    self.decoder:add(nn.View(1024,2,4,3))
    self.decoder:add(nn.VolumetricBatchNormalization(1024))    
    self.decoder:add(nn.PReLU())
    
    self.decoder:add(nn.VolumetricFullConvolution(1024, 512, kT-1, kW-1, kH-1, dT, dW, dH, padT-1, padW-1, padH-1))
