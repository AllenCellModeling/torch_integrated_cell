...
function Model:createAutoencoder()
    ...
    self.encoder:add(nn.VolumetricConvolution(self.nChIn, nconv, kT, kW, kH, dT, dW, dH, padT, padW, padH))
    self.encoder:add(nn.VolumetricBatchNormalization(nconv))
    self.encoder:add(nn.ReLU(true))
    
    self.encoder:add(nn.VolumetricConvolution(nconv, nconv*2, kT, kW, kH, dT, dW, dH, padT, padW, padH))
    self.encoder:add(nn.VolumetricBatchNormalization(nconv*2))
    self.encoder:add(nn.ReLU(true))
    
    self.encoder:add(nn.VolumetricConvolution(nconv*2, nconv*3, kT, kW, kH, dT, dW, dH, padT, padW, padH))
    self.encoder:add(nn.VolumetricBatchNormalization(nconv*3))
    self.encoder:add(nn.ReLU(true))
    
    self.encoder:add(nn.VolumetricConvolution(nconv*3, nconv*4, kT, kW, kH, dT, dW, dH, padT, padW, padH))
    self.encoder:add(nn.VolumetricBatchNormalization(nconv*4))
    self.encoder:add(nn.ReLU(true))
    
    self.encoder:add(nn.VolumetricConvolution(nconv*4, nconv*5, kT, kW, kH, dT, dW, dH, padT, padW, padH))
    self.encoder:add(nn.View(nconv*5*2*4*3))
    self.encoder:add(nn.BatchNormalization(nconv*5*2*4*3))
    self.encoder:add(nn.ReLU(true))
    ...
    if self.nClasses > 0 then
        ...
        pred_label:add(nn.Linear(nconv*5*2*4*3, self.nClasses))
    ...
    -- output for the structure features
        ...
        pred_other:add(nn.Linear(nconv*5*2*4*3, self.nOther))
    ...
    if self.nLatentDims > 0 then
        ...
        noise:add(nn.Linear(nconv*5*2*4*3, self.nLatentDims))
    ...
    self.decoder:add(nn.Linear(self.nClasses + self.nOther + self.nLatentDims, nconv*5*2*4*3))

    self.decoder:add(nn.View(nconv*5,2,4,3))
    self.decoder:add(nn.VolumetricBatchNormalization(nconv*5))    
    self.decoder:add(nn.ReLU(true))
    
    self.decoder:add(nn.VolumetricFullConvolution(nconv*5, nconv*4, kT, kW, kH, dT, dW, dH, padT-1, padW-1, padH-1))
    self.decoder:add(nn.VolumetricBatchNormalization(nconv*4))
    self.decoder:add(nn.ReLU(true))
    
    self.decoder:add(nn.VolumetricFullConvolution(nconv*4, nconv*3, kT, kW, kH, dT, dW, dH, padT-1, padW-1, padH-1))
    self.decoder:add(nn.VolumetricBatchNormalization(nconv*3))
    self.decoder:add(nn.ReLU(true))
    
    self.decoder:add(nn.VolumetricFullConvolution(nconv*3, nconv*2, kT, kW, kH, dT, dW, dH, padT-1, padW-1, padH-1))
    self.decoder:add(nn.VolumetricBatchNormalization(nconv*2))
    self.decoder:add(nn.ReLU(true))
    
    self.decoder:add(nn.VolumetricFullConvolution(nconv*2, nconv, kT, kW, kH, dT, dW, dH, padT-1, padW-1, padH-1))
    self.decoder:add(nn.VolumetricBatchNormalization(nconv))
    self.decoder:add(nn.ReLU(true))
    
    self.decoder:add(nn.VolumetricFullConvolution(nconv, self.nChOut, kT, kW, kH, dT, dW, dH, padT-1, padW-1, padH-1))
...
function Model:createGenAdversary()
    ...
    self.adversaryGen:add(nn.VolumetricConvolution(ndf * 4, ndf * 4, kT, kW, kH, dT, dW, dH, padT, padW, padH))
    self.adversaryGen:add(nn.VolumetricBatchNormalization(ndf * 4))
    self.adversaryGen:add(nn.LeakyReLU(0.2, true))
